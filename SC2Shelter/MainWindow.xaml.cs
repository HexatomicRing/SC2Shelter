...
namespace SC2Shelter
{
	...
	public partial class MainWindow
	{
		...
		private void Print(string text)
		{
			Dispatcher.Invoke(() =>
			{
				ConsoleBox.AppendText($"{DateTime.Now.ToString("yyyy:MM:dd HH:mm:ss")}: {text}{Environment.NewLine}");
			});
		}
		...
		/// <returns></returns>
		private async Task<List<string>> SearchErrorFiles(string dir)
		{
			List<string> ret = new List<string>();
			string[] files = Directory.GetFiles(dir, "*.s2ml", SearchOption.AllDirectories);
